...
class PatchDropout(torch.nn.Module):
    ...
    def __init__(
        ...
        self.n_keep = int(
            self.tokens_per_frame * (1 - p)
        )  # number of frames to keep per patch (if tubelet sampling is used)
    ...
    def get_mask(self, x):
        if self.sampling == "uniform":
            ...
            raise NotImplementedError(
                f"PatchDropout does not support {self.sampling} sampling"
            )
            return None
    ...
    def tubelet_uniform_mask(self, x):
        ...
        values_to_add = self.tokens_per_frame * torch.arange(
            0, self.num_frames
        ).repeat_interleave(keep).to(x.device)
        ...
        patch_mask = (
            patch_mask + 1
        )  # add 1 to account for CLS token (assumes it is leading token)

        restore_mask = (
            repeated_restore_mask + 1
        )  # add 1 to account for CLS token (assumes it is leading token)
        ...
        if not self.token_shuffling:
            ...
            raise NotImplementedError(
                "Token shuffling is not implemented for tubelet_uniform_mask"
            )
...
class GatedTimeVarAttention(VarAttention):
    def __init__(
        ...
        *args,
        ...
        patches_per_frame_after_dropout=196,
        **kwargs,
    ):
...
class SpaceTimeBlock(nn.Module):
    def __init__(
        ...
        if attention_style == "freeze-first-frame":
            self.timeattn = GatedTimeVarAttention(
                ...
                patches_per_frame_after_dropout=patches_per_frame_after_dropout,
            )
        else:
    ...
    def forward(
        ...
        if (
            self.attention_style == "frozen-in-time"
            or self.attention_style == "freeze-first-frame"
        ):
...
class SpaceTimeTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.patches_per_frame_after_dropout = int(
            round(self.patches_per_frame * (1 - self.patch_drop_rate))
        )
        ...
        if freeze_first_frame:
            self.first_frame_temporal_embed = nn.Parameter(
