...
start_times = df['fix_start']
end_times = df['stop']
captions = df['text']
global_captions = df['global_text']
...
for i in range(3):
    ...
    train_df.to_csv(os.path.join("train_{}.csv".format(i)), index=False)
    val_df.to_csv(os.path.join("test_{}.csv".format(i)), index=False)
